...
public class IntensifyCache<K, V, L> {
     ...
     */
    public IntensifyCache(int maxSize) {
        if (maxSize <= 0) {
    ...
    public final V get(K key) {
        ...
        V alternative = alternative(key, level);
        if (alternative != null) {
            return alternative;
        }
    ...
    /**
     * get(key) returns null, this will called before create(key) and would not
     * add into this caches.
     *
     * @param key   the cache key.
     * @param level the cache level.
     * @return alternative.
     */
    protected V alternative(K key, L level) {
        return null;
    }
